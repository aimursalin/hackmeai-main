...
import { teamMembers as defaultMembers, TeamMember } from "@/data/siteData";
import { User, Briefcase, Star, Plus, Trash2, Edit, Loader2 } from "lucide-react";
      ...
      try {
        ...
        if (data && data.length > 0) {
          const mappedMembers: TeamMember[] = data.map((d: any) => ({
            ...
            image: d.profiles.avatar_url,
            stars: d.stars,
            tag: d.profiles.role === 'admin' ? 'ELITE' : 'PRO',
            experience: [
              { role: d.profiles.role, company: 'HackMeAI', duration: '5+ years' }
            ],
            skills: ['UI/UX', 'Strategy'],
            successRate: '98%',
            statLabel: 'Projects',
            statValue: '150+'
          }));
