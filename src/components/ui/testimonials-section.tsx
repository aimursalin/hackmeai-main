...
export function TestimonialsSection() {
	return (
		<section className="relative w-full pt-10 pb-20 px-4 overflow-hidden">
			<div aria-hidden className="absolute inset-0 isolate z-0 contain-strict">
				...
				<div className="bg-[radial-gradient(50%_50%_at_50%_50%,rgba(255,255,255,0.04)_0,rgba(255,255,255,0.01)_80%,transparent_100%)] absolute top-0 left-0 h-[80rem] w-[15rem] [translate:5%_-50%] -rotate-45 rounded-full" />
				...
				<div className="relative grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
					{testimonials.map(({ name, role, company, quote, image }, index) => (
						<motion.div
							...
							key={index}
							className="border-white/10 relative grid grid-cols-[auto_1fr] gap-x-3 overflow-hidden border border-dashed p-4 bg-white/[0.02] backdrop-blur-sm"
						>
							...
							<div>
								...
								<blockquote className="mt-3 text-left">
									<p className="text-muted-foreground text-sm font-light tracking-wide italic">
										"{quote}"
									</p>
								</blockquote>
							</div>
